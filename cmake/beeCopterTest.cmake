# ============================================================================
# beeCopter Test Infrastructure
# ============================================================================
# CTest integration with labels, timeouts, and resource locking.
#
# Usage:
#   include(beeCopterTest)
#   add_beeCopter_test(MyTest LABELS Unit)
#   add_beeCopter_test(MyIntegrationTest LABELS Integration RESOURCE_LOCK MockLink)
...
set(beeCopter_TEST_PARALLEL_LEVEL ${_num_cores} CACHE STRING "Number of parallel test jobs")

set(beeCopter_TEST_TIMEOUT_UNIT 60 CACHE STRING "Timeout for unit tests (seconds)")
set(beeCopter_TEST_TIMEOUT_INTEGRATION 120 CACHE STRING "Timeout for integration tests (seconds)")
set(beeCopter_TEST_TIMEOUT_SLOW 180 CACHE STRING "Timeout for slow tests (seconds)")
set(beeCopter_TEST_TIMEOUT_DEFAULT 90 CACHE STRING "Default test timeout (seconds)")
...
add_custom_target(check
    COMMAND ${CMAKE_CTEST_COMMAND} --output-on-failure --parallel ${beeCopter_TEST_PARALLEL_LEVEL}
    USES_TERMINAL
...
add_custom_target(check-unit
    COMMAND ${CMAKE_CTEST_COMMAND} -L Unit --output-on-failure --parallel ${beeCopter_TEST_PARALLEL_LEVEL}
    USES_TERMINAL
...
add_custom_target(check-integration
    COMMAND ${CMAKE_CTEST_COMMAND} -L Integration --output-on-failure --parallel ${beeCopter_TEST_PARALLEL_LEVEL}
    USES_TERMINAL
...
add_custom_target(check-fast
    COMMAND ${CMAKE_CTEST_COMMAND} -LE Slow --output-on-failure --parallel ${beeCopter_TEST_PARALLEL_LEVEL}
    USES_TERMINAL
...
add_custom_target(check-ci
    COMMAND ${CMAKE_CTEST_COMMAND} -LE "Flaky|Network" --output-on-failure --parallel ${beeCopter_TEST_PARALLEL_LEVEL}
    USES_TERMINAL
...
foreach(_category MissionManager Vehicle Utilities MAVLink Comms)
    ...
    add_custom_target(check-${_target_suffix}
        COMMAND ${CMAKE_CTEST_COMMAND} -L ${_category} --output-on-failure --parallel ${beeCopter_TEST_PARALLEL_LEVEL}
        USES_TERMINAL
...
set(_beeCopter_check_targets check check-unit check-integration check-fast check-ci)
foreach(_category MissionManager Vehicle Utilities MAVLink Comms)
    string(TOLOWER ${_category} _target_suffix)
    list(APPEND _beeCopter_check_targets check-${_target_suffix})
endforeach()

# Ensure all check targets build the main executable first
foreach(_target IN LISTS _beeCopter_check_targets)
...
#
# Example:
#   add_beeCopter_test(ParameterManagerTest LABELS Integration Vehicle SERIAL)

function(add_beeCopter_test test_name)
    ...
    elseif("Slow" IN_LIST ARG_LABELS)
        set(_timeout ${beeCopter_TEST_TIMEOUT_SLOW})
    elseif("Integration" IN_LIST ARG_LABELS)
        set(_timeout ${beeCopter_TEST_TIMEOUT_INTEGRATION})
    elseif("Unit" IN_LIST ARG_LABELS)
        set(_timeout ${beeCopter_TEST_TIMEOUT_UNIT})
    else()
        set(_timeout ${beeCopter_TEST_TIMEOUT_DEFAULT})
    endif()
