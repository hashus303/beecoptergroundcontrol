...
beeCopter_LOGGING_CATEGORY(beeCopterCameraParamIOLog, "Camera.beeCopterCameraParamIO")
beeCopter_LOGGING_CATEGORY(beeCopterCameraParamIOVerbose, "Camera.beeCopterCameraParamIO:verbose")

namespace {
    constexpr int kMaxRetries = 3;
}

beeCopterCameraParamIO::beeCopterCameraParamIO(MavlinkCameraControl *control, Fact *fact, Vehicle *vehicle)
    : QObject(control)
    , _control(control)
    , _fact(fact)
    , _vehicle(vehicle)
{
    qCDebug(beeCopterCameraParamIOLog) << this;
    ...
    if (_fact->writeOnly()) {
        ...
        (void) connect(&_paramRequestTimer, &QTimer::timeout, this, &beeCopterCameraParamIO::_paramRequestTimeout);
    }
    (void) connect(&_paramWriteTimer,   &QTimer::timeout, this, &beeCopterCameraParamIO::_paramWriteTimeout);
    (void) connect(_fact, &Fact::rawValueChanged, this, &beeCopterCameraParamIO::_factChanged);
    (void) connect(_fact, &Fact::containerRawValueChanged, this, &beeCopterCameraParamIO::_containerRawValueChanged);
    ...
    switch (_fact->type()) {
        ...
        case FactMetaData::valueTypeCustom:
            ...
            break;
        default:
            qCWarning(beeCopterCameraParamIOLog) << "Unsupported fact type" << _fact->type() << "for" << _fact->name();
            Q_FALLTHROUGH();
...
beeCopterCameraParamIO::~beeCopterCameraParamIO()
{
    qCDebug(beeCopterCameraParamIOLog) << this;
}

void beeCopterCameraParamIO::_paramRequestTimeout()
{
    if (++_requestRetries > kMaxRetries) {
        qCWarning(beeCopterCameraParamIOLog) << "No response for param request:" << _fact->name();
        if (!_done) {
            _done = true;
            _control->_paramDone();
        }
        ...
        // Request it again
        qCDebug(beeCopterCameraParamIOLog) << "Param request retry:" << _fact->name();
        paramRequest(false);
        _paramRequestTimer.start();
    }
}

void beeCopterCameraParamIO::_paramWriteTimeout()
{
    if (++_sentRetries > kMaxRetries) {
        qCWarning(beeCopterCameraParamIOLog) << "No response for param set:" << _fact->name();
        _updateOnSet = false;
    } else {
        // Send it again
        qCDebug(beeCopterCameraParamIOLog) << "Param set retry:" << _fact->name() << _sentRetries;
        _sendParameter();
        _paramWriteTimer.start();
    }
}

void beeCopterCameraParamIO::_factChanged(const QVariant &value)
{
    Q_UNUSED(value);

    if (!_forceUIUpdate) {
        qCDebug(beeCopterCameraParamIOLog) << "UI Fact" << _fact->name() << "changed to" << value;
        _control->factChanged(_fact);
    }
}

void beeCopterCameraParamIO::_containerRawValueChanged(const QVariant &value)
{
    Q_UNUSED(value);

    if (!_fact->readOnly()) {
        qCDebug(beeCopterCameraParamIOLog) << "Update Fact from camera" << _fact->name();
        _sentRetries = 0;
        _sendParameter();
    }
}

void beeCopterCameraParamIO::_sendParameter()
{
    ...
    if (sharedLink) {
        ...
        beeCopterMAVLink::param_ext_union_t union_value{};
        ...
        default:
            qCCritical(beeCopterCameraParamIOLog) << "Unsupported fact type" << factType << "for" << _fact->name();
            Q_FALLTHROUGH();
        case FactMetaData::valueTypeInt32:
        ...
        if (!ok) {
            qCCritical(beeCopterCameraParamIOLog) << "Invalid value for" << _fact->name() << ":" << _fact->rawValue();
        }
...
void beeCopterCameraParamIO::handleParamAck(const mavlink_param_ext_ack_t &ack)
{
    ...
    case PARAM_ACK_IN_PROGRESS:
        ...
        qCDebug(beeCopterCameraParamIOLog) << "Param set in progress:" << _fact->name();
        _paramWriteTimer.start();
        break;
    case PARAM_ACK_FAILED:
        if (++_sentRetries < kMaxRetries) {
            // Try again
            qCWarning(beeCopterCameraParamIOLog) << "Param set failed:" << _fact->name() << _sentRetries;
            _paramWriteTimer.start();
        }
        break;
    case PARAM_ACK_VALUE_UNSUPPORTED:
        qCWarning(beeCopterCameraParamIOLog) << "Param set unsuported:" << _fact->name();
        Q_FALLTHROUGH();
    default: {
...
QVariant beeCopterCameraParamIO::_valueFromMessage(const char *value, uint8_t param_type)
{
    QVariant var;
    beeCopterMAVLink::param_ext_union_t u{};
    ...
    switch (param_type) {
        ...
        default:
            var = QVariant(0);
            qCCritical(beeCopterCameraParamIOLog) << "Invalid param_type used for camera setting:" << param_type;
            break;
    }
...
void beeCopterCameraParamIO::handleParamValue(const mavlink_param_ext_value_t &value)
{
    ...
    qCDebug(beeCopterCameraParamIOLog) << QStringLiteral("handleParamValue() %1 %2").arg(_fact->name(), _fact->rawValueString());
}

void beeCopterCameraParamIO::paramRequest(bool reset)
{
    ...
    qCDebug(beeCopterCameraParamIOLog) << "Request parameter:" << _fact->name();
    SharedLinkInterfacePtr sharedLink = _vehicle->vehicleLinkManager()->primaryLink().lock();
    if (sharedLink) {
...
void beeCopterCameraParamIO::sendParameter(bool updateUI)
{
    qCDebug(beeCopterCameraParamIOLog) << "Send Fact" << _fact->name();
    _sentRetries = 0;
    _updateOnSet = updateUI;
    _sendParameter();
}

void beeCopterCameraParamIO::setParamRequest()
{
    if (!_fact->writeOnly()) {
