...
class beeCopterCameraManagerTest;
class beeCopterVideoStreamInfo;
class SimulatedCameraControl;

/// Camera Manager
class beeCopterCameraManager : public QObject
{
    Q_OBJECT
    QML_ELEMENT
    QML_UNCREATABLE("")
    ...
    friend class beeCopterCameraManagerTest;

public:
    explicit beeCopterCameraManager(Vehicle* vehicle);
    ~beeCopterCameraManager();

    struct CameraStruct {
        CameraStruct(beeCopterCameraManager* manager_, uint8_t compID_, Vehicle* vehicle_);
        ~CameraStruct();
        ...
        QPointer<beeCopterCameraManager> manager;
    ...
    beeCopterVideoStreamInfo* currentStreamInstance();
    beeCopterVideoStreamInfo* thermalStreamInstance();
