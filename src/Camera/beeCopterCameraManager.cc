...
beeCopter_LOGGING_CATEGORY(CameraManagerLog, "Camera.beeCopterCameraManager")
...
QVariantList beeCopterCameraManager::_cameraList;
...
{
    auto* mgr = static_cast<beeCopterCameraManager*>(user);
...
beeCopterCameraManager::CameraStruct::CameraStruct(beeCopterCameraManager *manager_, uint8_t compID_, Vehicle *vehicle_)
    : compID(compID_)
    , vehicle(vehicle_)
    , manager(manager_)
{
...
beeCopterCameraManager::CameraStruct::~CameraStruct()
{
    qCDebug(CameraManagerLog) << this;
}
...
beeCopterCameraManager::beeCopterCameraManager(Vehicle *vehicle)
    : QObject(vehicle)
    , _vehicle(vehicle)
    , _simulatedCameraControl(new SimulatedCameraControl(vehicle, this))
{
    ...
    (void) connect(MultiVehicleManager::instance(), &MultiVehicleManager::parameterReadyVehicleAvailableChanged, this, &beeCopterCameraManager::_vehicleReady);
    (void) connect(_vehicle, &Vehicle::mavlinkMessageReceived, this, &beeCopterCameraManager::_mavlinkMessageReceived);
    (void) connect(&_camerasLostHeartbeatTimer, &QTimer::timeout, this, &beeCopterCameraManager::_checkForLostCameras);
...
beeCopterCameraManager::~beeCopterCameraManager()
{
...
void beeCopterCameraManager::setCurrentCamera(int sel)
{
...
void beeCopterCameraManager::_vehicleReady(bool ready)
{
    ...
    (void) connect(JoystickManager::instance(), &JoystickManager::activeJoystickChanged, this, &beeCopterCameraManager::_activeJoystickChanged, Qt::UniqueConnection);
}

void beeCopterCameraManager::_mavlinkMessageReceived(const mavlink_message_t &message)
{
...
void beeCopterCameraManager::_handleHeartbeat(const mavlink_message_t &message)
{
    const QString sCompID = QString::number(message.compid);

    if (!_cameraInfoRequest.contains(sCompID)) {
        qCDebug(CameraManagerLog) << "Heartbeat from" << beeCopterMAVLink::compIdToString(message.compid);
    ...
    if (pInfo->lastHeartbeat.elapsed() > kSilentTimeoutMs) {
        qCDebug(CameraManagerLog) << "Camera" << beeCopterMAVLink::compIdToString(message.compid) << "reappeared after being silent. Resetting retry count and requesting info.";
        pInfo->retryCount = 0;
...
MavlinkCameraControl *beeCopterCameraManager::currentCameraInstance()
{
...
beeCopterVideoStreamInfo *beeCopterCameraManager::currentStreamInstance()
{
    MavlinkCameraControl *pCamera = currentCameraInstance();
    if (pCamera) {
        beeCopterVideoStreamInfo *pInfo = pCamera->currentStreamInstance();
        return pInfo;
    }
    return nullptr;
}

beeCopterVideoStreamInfo *beeCopterCameraManager::thermalStreamInstance()
{
    MavlinkCameraControl *pCamera = currentCameraInstance();
    if (pCamera) {
        beeCopterVideoStreamInfo *pInfo = pCamera->thermalStreamInstance();
        return pInfo;
    }
    return nullptr;
}

MavlinkCameraControl *beeCopterCameraManager::_findCamera(int id)
{
...
void beeCopterCameraManager::_addCameraControlToLists(MavlinkCameraControl *cameraControl)
{
...
void beeCopterCameraManager::_handleCameraInfo(const mavlink_message_t& message)
{
    const QString sCompID = QString::number(message.compid);
    if (!_cameraInfoRequest.contains(sCompID)) {
        qCDebug(CameraManagerLog) << "Ignoring - Camera info not requested for component" << beeCopterMAVLink::compIdToString(message.compid);
        return;
    }
    if (_cameraInfoRequest[sCompID]->infoReceived) {
        qCDebug(CameraManagerLog) << "Ignoring - Already received camera info for component" << beeCopterMAVLink::compIdToString(message.compid);
        return;
    }

    mavlink_camera_information_t info{};
    mavlink_msg_camera_information_decode(&message, &info);
    qCDebug(CameraManagerLog) << "Camera information received from" << beeCopterMAVLink::compIdToString(message.compid)
    ...
    if (pCamera) {
        ...
        qCDebug(CameraManagerLog) << "Success for compId" << beeCopterMAVLink::compIdToString(message.compid) << "- reset retry counter";
    }
...
void beeCopterCameraManager::_checkForLostCameras()
{
    QList<QString> stale;
    ...
    for (const QString& key : std::as_const(stale)) {
        ...
        if (pCamera) {
            ...
            if (idx >= 0) {
                qCDebug(CameraManagerLog) << "Removing lost camera" << beeCopterMAVLink::compIdToString(pInfo->compID);
                removedAny = true;
...
void beeCopterCameraManager::_handleCaptureStatus(const mavlink_message_t &message)
{
...
void beeCopterCameraManager::_handleStorageInfo(const mavlink_message_t &message)
{
    MavlinkCameraControl *pCamera = _findCamera(message.compid);
    if (pCamera) {
...
void beeCopterCameraManager::_handleCameraSettings(const mavlink_message_t& message)
{
...
void beeCopterCameraManager::_handleParamAck(const mavlink_message_t &message)
{
    MavlinkCameraControl *pCamera = _findCamera(message.compid);
    if (pCamera) {
...
void beeCopterCameraManager::_handleParamValue(const mavlink_message_t &message)
{
    MavlinkCameraControl *pCamera = _findCamera(message.compid);
    if (pCamera) {
...
void beeCopterCameraManager::_handleVideoStreamInfo(const mavlink_message_t &message)
{
    MavlinkCameraControl *pCamera = _findCamera(message.compid);
    if (pCamera) {
...
void beeCopterCameraManager::_handleVideoStreamStatus(const mavlink_message_t &message)
{
    MavlinkCameraControl *pCamera = _findCamera(message.compid);
    if (pCamera) {
...
void beeCopterCameraManager::_handleBatteryStatus(const mavlink_message_t &message)
{
    MavlinkCameraControl *pCamera = _findCamera(message.compid);
    if (pCamera) {
...
void beeCopterCameraManager::_handleTrackingImageStatus(const mavlink_message_t &message)
{
    MavlinkCameraControl *pCamera = _findCamera(message.compid);
    if (pCamera) {
...
static void _handleCameraInfoRetry(beeCopterCameraManager::CameraStruct *cameraInfo);
...
{
    auto *cameraInfo = static_cast<beeCopterCameraManager::CameraStruct*>(resultHandlerData);

    if (ack.result != MAV_RESULT_ACCEPTED) {
        qCDebug(CameraManagerLog) << "MAV_CMD_REQUEST_CAMERA_INFORMATION failed. compId" << beeCopterMAVLink::compIdToString(cameraInfo->compID)
                                    << "Result:" << beeCopterMAVLink::mavResultToString(ack.result)
...
static void _requestCameraInfoMessageResultHandler(void *resultHandlerData, MAV_RESULT result, Vehicle::RequestMessageResultHandlerFailureCode_t failureCode, [[maybe_unused]] const mavlink_message_t &message)
{
    ...
    if (result != MAV_RESULT_ACCEPTED) {
        qCDebug(CameraManagerLog) << "MAV_CMD_REQUEST_MESSAGE:MAVLINK_MSG_ID_CAMERA_INFORMATION failed. compId" << beeCopterMAVLink::compIdToString(cameraInfo->compID)
                                    << "Result:" << beeCopterMAVLink::mavResultToString(result)
...
static void _requestCameraInfoHelper(beeCopterCameraManager *manager, beeCopterCameraManager::CameraStruct *pInfo)
{
    // Give up after max attempts
    if (pInfo->retryCount >= kMaxRetryCount) {
        qCDebug(CameraManagerLog) << "Giving up requesting camera info after" << pInfo->retryCount << "attempts for compId" << beeCopterMAVLink::compIdToString(pInfo->compID);
        return;
    }

    // Alternate between REQUEST_MESSAGE and REQUEST_CAMERA_INFORMATION
    if ((pInfo->retryCount % 2) == 0) {
        qCDebug(CameraManagerLog) << "Using MAV_CMD_REQUEST_MESSAGE:CAMERA_INFORMATION for compId" << beeCopterMAVLink::compIdToString(pInfo->compID);
        ...
        qCDebug(CameraManagerLog) << "Using MAV_CMD_REQUEST_CAMERA_INFORMATION for compId" << beeCopterMAVLink::compIdToString(pInfo->compID);
...
static void _handleCameraInfoRetry(beeCopterCameraManager::CameraStruct *cameraInfo)
{
    ...
    beeCopterCameraManager *manager = cameraInfo->manager;
    if (!manager) {
        qCDebug(CameraManagerLog) << "manager is unavailable for compId" << beeCopterMAVLink::compIdToString(cameraInfo->compID);
        return;
    }
    ...
    if ((cameraInfo->retryCount >= 2) && ((cameraInfo->retryCount % 2) == 0)) {
        ...
        qCDebug(CameraManagerLog) << "Waiting" << delaySeconds << "seconds before retry for compId" << beeCopterMAVLink::compIdToString(cameraInfo->compID);
        ...
        QPointer<beeCopterCameraManager> mgrGuard(manager);
...
void beeCopterCameraManager::_requestCameraInfo(CameraStruct *pInfo)
{
    if (!pInfo) {
        return;
    }
...
void beeCopterCameraManager::_activeJoystickChanged(Joystick *joystick)
{
    qCDebug(CameraManagerLog) << "Joystick changed";
    if (_activeJoystick) {
        (void) disconnect(_activeJoystick, &Joystick::stepZoom,            this, &beeCopterCameraManager::_stepZoom);
        (void) disconnect(_activeJoystick, &Joystick::startContinuousZoom, this, &beeCopterCameraManager::_startZoom);
        (void) disconnect(_activeJoystick, &Joystick::stopContinuousZoom,  this, &beeCopterCameraManager::_stopZoom);
        (void) disconnect(_activeJoystick, &Joystick::stepCamera,          this, &beeCopterCameraManager::_stepCamera);
        (void) disconnect(_activeJoystick, &Joystick::stepStream,          this, &beeCopterCameraManager::_stepStream);
        (void) disconnect(_activeJoystick, &Joystick::triggerCamera,       this, &beeCopterCameraManager::_triggerCamera);
        (void) disconnect(_activeJoystick, &Joystick::startVideoRecord,    this, &beeCopterCameraManager::_startVideoRecording);
        (void) disconnect(_activeJoystick, &Joystick::stopVideoRecord,     this, &beeCopterCameraManager::_stopVideoRecording);
        (void) disconnect(_activeJoystick, &Joystick::toggleVideoRecord,   this, &beeCopterCameraManager::_toggleVideoRecording);
    }

    _activeJoystick = joystick;

    if (_activeJoystick) {
        (void) connect(_activeJoystick, &Joystick::stepZoom,            this, &beeCopterCameraManager::_stepZoom, Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::startContinuousZoom, this, &beeCopterCameraManager::_startZoom, Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::stopContinuousZoom,  this, &beeCopterCameraManager::_stopZoom, Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::stepCamera,          this, &beeCopterCameraManager::_stepCamera, Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::stepStream,          this, &beeCopterCameraManager::_stepStream, Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::triggerCamera,       this, &beeCopterCameraManager::_triggerCamera, Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::startVideoRecord,    this, &beeCopterCameraManager::_startVideoRecording, Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::stopVideoRecord,     this, &beeCopterCameraManager::_stopVideoRecording, Qt::UniqueConnection);
        (void) connect(_activeJoystick, &Joystick::toggleVideoRecord,   this, &beeCopterCameraManager::_toggleVideoRecording, Qt::UniqueConnection);
    }
}

void beeCopterCameraManager::_triggerCamera()
{
    MavlinkCameraControl *pCamera = currentCameraInstance();
    if (pCamera) {
        pCamera->takePhoto();
...
void beeCopterCameraManager::_startVideoRecording()
{
    MavlinkCameraControl *pCamera = currentCameraInstance();
    if (pCamera) {
...
void beeCopterCameraManager::_stopVideoRecording()
{
    MavlinkCameraControl *pCamera = currentCameraInstance();
    if (pCamera) {
...
void beeCopterCameraManager::_toggleVideoRecording()
{
    MavlinkCameraControl *pCamera = currentCameraInstance();
    if (pCamera) {
...
void beeCopterCameraManager::_stepZoom(int direction)
{
...
void beeCopterCameraManager::_startZoom(int direction)
{
...
void beeCopterCameraManager::_stopZoom()
{
...
void beeCopterCameraManager::_stepCamera(int direction)
{
...
void beeCopterCameraManager::_stepStream(int direction)
{
    if (_lastCameraChange.elapsed() > 1000) {
        _lastCameraChange.start();
        MavlinkCameraControl *pCamera = currentCameraInstance();
...
const QVariantList &beeCopterCameraManager::cameraList() const
{
...
void beeCopterCameraManager::requestCameraFovForComp(int compId) {
    if (!_vehicle) {
...
//-----------------------------------------------------------------------------
double beeCopterCameraManager::aspectForComp(int compId) const {
...
double beeCopterCameraManager::currentCameraAspect(){
    if (auto* cam = currentCameraInstance()) {
        return aspectForComp(cam->compID());
    }
    return std::numeric_limits<double>::quiet_NaN();
}
void beeCopterCameraManager::_handleCameraFovStatus(const mavlink_message_t& message)
{
    mavlink_camera_fov_status_t fov{};
    mavlink_msg_camera_fov_status_decode(&message, &fov);
...
void beeCopterCameraManager::_setCurrentZoomLevel(int level)
{
...
int beeCopterCameraManager::currentZoomLevel() const
{
