...
beeCopterMapPolygon::beeCopterMapPolygon(QObject* parent)
    : QObject               (parent)
    , _dirty                (false)
    , _centerDrag           (false)
...
beeCopterMapPolygon::beeCopterMapPolygon(const beeCopterMapPolygon& other, QObject* parent)
    : QObject               (parent)
    , _dirty                (false)
    , _centerDrag           (false)
    , _ignoreCenterUpdates  (false)
...
beeCopterMapPolygon::~beeCopterMapPolygon()
{
    beeCopterApp()->removeCompressedSignal(QMetaMethod::fromSignal(&beeCopterMapPolygon::pathChanged));
}

void beeCopterMapPolygon::_init(void)
{
    connect(&_polygonModel, &QmlObjectListModel::dirtyChanged, this, &beeCopterMapPolygon::_polygonModelDirtyChanged);
    connect(&_polygonModel, &QmlObjectListModel::countChanged, this, &beeCopterMapPolygon::_polygonModelCountChanged);
    connect(&_polygonModel, &QmlObjectListModel::modelReset, this, [this]() {
        emit pathChanged();
        emit centerChanged(_center);
    });

    connect(this, &beeCopterMapPolygon::pathChanged,  this, &beeCopterMapPolygon::_updateCenter);
    connect(this, &beeCopterMapPolygon::countChanged, this, &beeCopterMapPolygon::isValidChanged);
    connect(this, &beeCopterMapPolygon::countChanged, this, &beeCopterMapPolygon::isEmptyChanged);

}

const beeCopterMapPolygon& beeCopterMapPolygon::operator=(const beeCopterMapPolygon& other)
{
    clear();
...
void beeCopterMapPolygon::clear(void)
{
    ...
    // to be a bug in beeCopterMapPolygon which causes it to not be redrawn if the list is empty. So
...
void beeCopterMapPolygon::adjustVertex(int vertexIndex, const QGeoCoordinate coordinate)
{
    _polygonPath[vertexIndex] = QVariant::fromValue(coordinate);
    _polygonModel.value<beeCopterQGeoCoordinate*>(vertexIndex)->setCoordinate(coordinate);
    if (!_centerDrag) {
...
void beeCopterMapPolygon::setDirty(bool dirty)
{
    if (_dirty != dirty) {
        _dirty = dirty;
        if (!dirty) {
...
QGeoCoordinate beeCopterMapPolygon::_coordFromPointF(const QPointF& point) const
{
...
QPointF beeCopterMapPolygon::_pointFFromCoord(const QGeoCoordinate& coordinate) const
{
    if (_polygonPath.count() > 0) {
        double y, x, down;
        QGeoCoordinate tangentOrigin = _polygonPath[0].value<QGeoCoordinate>();
...
QPolygonF beeCopterMapPolygon::_toPolygonF(void) const
{
    QPolygonF polygon;
...
bool beeCopterMapPolygon::containsCoordinate(const QGeoCoordinate& coordinate) const
{
    if (_polygonPath.count() > 2) {
...
void beeCopterMapPolygon::setPath(const QList<QGeoCoordinate>& path)
{
    _polygonPath.clear();
    _polygonModel.clearAndDeleteContents();
    for(const QGeoCoordinate& coord: path) {
        _polygonPath.append(QVariant::fromValue(coord));
        _polygonModel.append(new beeCopterQGeoCoordinate(coord, this));
    }
...
void beeCopterMapPolygon::setPath(const QVariantList& path)
{
    _polygonPath = path;

    _polygonModel.clearAndDeleteContents();
    for (int i=0; i<_polygonPath.count(); i++) {
        _polygonModel.append(new beeCopterQGeoCoordinate(_polygonPath[i].value<QGeoCoordinate>(), this));
    }
...
void beeCopterMapPolygon::saveToJson(QJsonObject& json)
{
...
bool beeCopterMapPolygon::loadFromJson(const QJsonObject& json, bool required, QString& errorString)
{
    errorString.clear();
    clear();
...
QList<QGeoCoordinate> beeCopterMapPolygon::coordinateList(void) const
{
...
void beeCopterMapPolygon::splitPolygonSegment(int vertexIndex)
{
    ...
    if (nextIndex == 0) {
        ...
        _polygonModel.insert(nextIndex, new beeCopterQGeoCoordinate(newVertex, this));
...
void beeCopterMapPolygon::appendVertex(const QGeoCoordinate& coordinate)
{
    _polygonPath.append(QVariant::fromValue(coordinate));
    _polygonModel.append(new beeCopterQGeoCoordinate(coordinate, this));
    if (!_deferredPathChanged) {
        // Only update the path once per event loop, to prevent lag-spikes
        _deferredPathChanged = true;
        QTimer::singleShot(0, this, [this]() {
...
void beeCopterMapPolygon::appendVertices(const QList<QGeoCoordinate>& coordinates)
{
...
void beeCopterMapPolygon::appendVertices(const QVariantList& varCoords)
{
...
void beeCopterMapPolygon::_polygonModelDirtyChanged(bool dirty)
{
    if (dirty) {
        setDirty(true);
    }
...
void beeCopterMapPolygon::removeVertex(int vertexIndex)
{
...
void beeCopterMapPolygon::_polygonModelCountChanged(int count)
{
    emit countChanged(count);
}

void beeCopterMapPolygon::_updateCenter(void)
{
...
void beeCopterMapPolygon::setCenter(QGeoCoordinate newCenter)
{
...
void beeCopterMapPolygon::setCenterDrag(bool centerDrag)
{
...
void beeCopterMapPolygon::setInteractive(bool interactive)
{
...
QGeoCoordinate beeCopterMapPolygon::vertexCoordinate(int vertex) const
{
    if (vertex >= 0 && vertex < _polygonPath.count()) {
        return _polygonPath[vertex].value<QGeoCoordinate>();
    } else {
        qWarning() << "beeCopterMapPolygon::vertexCoordinate bad vertex requested:count" << vertex << _polygonPath.count();
        return QGeoCoordinate();
    }
}

QList<QPointF> beeCopterMapPolygon::nedPolygon(void) const
{
...
void beeCopterMapPolygon::offset(double distance)
{
...
bool beeCopterMapPolygon::loadKMLOrSHPFile(const QString& file)
{
    QString errorString;
    ...
    if (polygons.isEmpty()) {
        beeCopterApp()->showAppMessage(tr("No polygons found in file"));
        return false;
    }
...
double beeCopterMapPolygon::area(void) const
{
...
void beeCopterMapPolygon::verifyClockwiseWinding(void)
{
    if (_polygonPath.count() <= 2) {
        return;
    }
...
void beeCopterMapPolygon::beginReset(void)
{
    _polygonModel.beginResetModel();
}

void beeCopterMapPolygon::endReset(void)
{
    _polygonModel.endResetModel();
}

QDomElement beeCopterMapPolygon::kmlPolygonElement(KMLDomDocument& domDocument)
{
#if 0
    <Polygon id="ID">
      <!-- specific to Polygon -->
...
void beeCopterMapPolygon::setTraceMode(bool traceMode)
{
...
void beeCopterMapPolygon::setShowAltColor(bool showAltColor){
...
void beeCopterMapPolygon::selectVertex(int index)
{
    ...
    if(-1 <= index && index < count()) {
        ...
        if (!beeCopterApp()->runningUnitTests()) {
            qWarning() << QString("beeCopterMapPolygon: Selected vertex index (%1) is out of bounds! "
