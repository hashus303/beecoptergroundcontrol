...
/// The beeCopterMapPolygon class provides a polygon which can be displayed on a map using a map visuals control.
/// It maintains a representation of the polygon on QVariantList and QmlObjectListModel format.
class beeCopterMapPolygon : public QObject
{
    Q_OBJECT
    QML_ELEMENT
    QML_UNCREATABLE("")
public:
    beeCopterMapPolygon(QObject* parent = nullptr);
    beeCopterMapPolygon(const beeCopterMapPolygon& other, QObject* parent = nullptr);

    ~beeCopterMapPolygon() override;

    const beeCopterMapPolygon& operator=(const beeCopterMapPolygon& other);
