...
beeCopter_LOGGING_CATEGORY(beeCopterCommandLineParserLog, "Utilities.beeCopterCommandLineParser")
...
static QStringList normalizeArgs(const QStringList &args)
{
    ...
    qCDebug(beeCopterCommandLineParserLog) << "Normalized arguments:" << out;
    return out;
}
...
CommandLineParseResult parseCommandLine()
{
    ...
    QCoreApplication::setApplicationName(QLatin1String(beeCopter_APP_NAME));
    QCoreApplication::setApplicationVersion(QLatin1String(beeCopter_APP_VERSION_STR));
    ...
    parser.setApplicationDescription(QStringLiteral(beeCopter_APP_DESCRIPTION));
    ...
    if (!out.unknownOptions.isEmpty()) {
        ...
        if (out.unknownOptions.contains(QLatin1String("unittest")) ||
...
#ifndef beeCopter_UNITTEST_BUILD
            ...
            qCWarning(beeCopterCommandLineParserLog) << out.errorString.value();
            return out;
#endif
        }
    ...
    if (parser.isSet(systemIdOpt)) {
        ...
        qCDebug(beeCopterCommandLineParserLog) << "System ID:" << systemId;
    }
    ...
    if (parser.isSet(loggingOpt)) {
        ...
        qCDebug(beeCopterCommandLineParserLog) << "Logging options:" << out.loggingOptions.value();
    }
    ...
    if (parser.isSet(unittestOpt)) {
        ...
        qCDebug(beeCopterCommandLineParserLog) << "Unit tests:" << (out.unitTests.isEmpty() ? QStringLiteral("all") : out.unitTests.join(QLatin1String(", ")));
    }
    ...
    if (parser.isSet(unittestStressOpt)) {
        ...
        qCDebug(beeCopterCommandLineParserLog) << "Stress test iterations:" << out.stressUnitTestsCount;
    }

    if (parser.isSet(unittestOutputOpt)) {
        out.unitTestOutput = parser.value(unittestOutputOpt);
        qCDebug(beeCopterCommandLineParserLog) << "Test output file:" << out.unitTestOutput.value();
    }

    if (parser.isSet(unittestLabelOpt)) {
        out.labelFilter = parser.value(unittestLabelOpt);
        qCDebug(beeCopterCommandLineParserLog) << "Label filter:" << out.labelFilter.value();
    }

    out.listTests = parser.isSet(listTestsOpt);
    if (out.listTests) {
        qCDebug(beeCopterCommandLineParserLog) << "List tests requested";
    }
#endif
    ...
    qCDebug(beeCopterCommandLineParserLog) << "Command line parsing completed successfully";
