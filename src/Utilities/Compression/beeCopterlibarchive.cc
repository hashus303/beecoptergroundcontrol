...
beeCopter_LOGGING_CATEGORY(beeCopterlibarchiveLog, "Utilities.beeCopterlibarchive")
...
void updateFormatState(struct archive* a)
{
    ...
    if (!t_formatState.formatName.isEmpty()) {
        qCDebug(beeCopterlibarchiveLog) << "Detected format:" << t_formatState.formatName
...
{
    static thread_local char readBuffer[beeCopterFileHelper::kBufferSizeMax];
...
la_int64_t deviceSkipCallback(struct archive*, void* clientData, la_int64_t request)
{
    ...
    char discardBuffer[beeCopterFileHelper::kBufferSizeMax];
...
QByteArray readArchiveToMemory(struct archive* a, qint64 expectedSize = 0, qint64 maxBytes = 0)
{
    ...
    char buffer[beeCopterFileHelper::kBufferSizeMax];
    la_ssize_t size;
    while ((size = archive_read_data(a, buffer, sizeof(buffer))) > 0) {
        if (maxBytes > 0 && (result.size() + size) > maxBytes) {
            qCWarning(beeCopterlibarchiveLog) << "Size limit exceeded:" << maxBytes << "bytes";
            return {};
        }
    ...
    if (size < 0) {
        qCWarning(beeCopterlibarchiveLog) << "Read error:" << archive_error_string(a);
        return {};
    }
...
bool writeArchiveEntryToFile(struct archive* a, const QString& outputPath)
{
    ...
    if (!outFile.open(QIODevice::WriteOnly)) {
        qCWarning(beeCopterlibarchiveLog) << "Failed to open output file:" << outputPath << outFile.errorString();
        return false;
    }
    ...
    while ((r = archive_read_data_block(a, &buff, &size, &offset)) == ARCHIVE_OK) {
        ...
        if (!outFile.seek(offset)) {
            qCWarning(beeCopterlibarchiveLog) << "Failed to seek:" << outFile.errorString();
            outFile.cancelWriting();
            return false;
        }
        if (outFile.write(static_cast<const char*>(buff), static_cast<qint64>(size)) != static_cast<qint64>(size)) {
            qCWarning(beeCopterlibarchiveLog) << "Failed to write output:" << outFile.errorString();
            outFile.cancelWriting();
            return false;
        }
    }
    ...
    if (!outFile.commit()) {
        qCWarning(beeCopterlibarchiveLog) << "Failed to commit output file:" << outFile.errorString();
        return false;
    }
...
bool decompressStreamToFile(struct archive* a, const QString& outputPath,
                            const beeCopterlibarchive::ProgressCallback& progress, qint64 totalSize, qint64 maxBytes)
{
    QSaveFile outFile(outputPath);
    if (!outFile.open(QIODevice::WriteOnly)) {
        qCWarning(beeCopterlibarchiveLog) << "Failed to open output file:" << outputPath << outFile.errorString();
    ...
    while ((size = archive_read_data(a, buffer, sizeof(buffer))) > 0) {
        ...
        if (progress) {
            ...
            if (!progress(bytesRead, totalSize)) {
                qCDebug(beeCopterlibarchiveLog) << "Decompression cancelled by user";
                outFile.cancelWriting();
                return false;
            }
        }
    ...
    if (size < 0) {
        qCWarning(beeCopterlibarchiveLog) << "Decompression error:" << archive_error_string(a);
        outFile.cancelWriting();
        return false;
    }
    ...
    qCDebug(beeCopterlibarchiveLog) << "Decompressed" << totalBytesWritten << "bytes to" << outputPath;
    return true;
}
                           ...
                           const beeCopterlibarchive::ProgressCallback& progress, qint64 totalSize, qint64 maxBytes)
{
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        QString outputPath = beeCopterFileHelper::joinPath(outputDirectoryPath, entryName);
        ...
        if (!canonicalOutput.startsWith(canonicalOutputDir + "/") && canonicalOutput != canonicalOutputDir) {
            qCWarning(beeCopterlibarchiveLog) << "Skipping path traversal attempt:" << currentFile;
            continue;
        }

        // Security: reject filenames with embedded null bytes (libarchive issue #2774)
        if (entryName.contains(QChar('\0'))) {
            qCWarning(beeCopterlibarchiveLog) << "Skipping path with embedded null byte";
            continue;
        }
        ...
        if (fileType == AE_IFLNK) {
            ...
            if (symlinkTarget) {
                ...
                if (QFileInfo(target).isAbsolute()) {
                    ...
                    qCWarning(beeCopterlibarchiveLog)
        ...
        if (r != ARCHIVE_OK) {
            qCWarning(beeCopterlibarchiveLog) << "Failed to write header for" << outputPath << ":"
        ...
        if (archive_entry_size(entry) > 0) {
            ...
            while ((r = archive_read_data_block(a, &buff, &size, &offset)) == ARCHIVE_OK) {
                ...
                if (archive_write_data_block(ext, buff, size, offset) != ARCHIVE_OK) {
                    qCWarning(beeCopterlibarchiveLog) << "Failed to write data:" << archive_error_string(ext);
                    success = false;
                    break;
                }
                ...
                if (progress) {
                    ...
                    if (!progress(compressedBytesRead, totalSize)) {
                        qCDebug(beeCopterlibarchiveLog) << "Extraction cancelled by user";
                        cancelled = true;
                        success = false;
                        break;
                    }
            ...
            if (r != ARCHIVE_EOF && r != ARCHIVE_OK) {
                qCWarning(beeCopterlibarchiveLog) << "Failed to read data:" << archive_error_string(a);
                success = false;
                break;
            }
        }
        ...
        if (archive_write_finish_entry(ext) != ARCHIVE_OK) {
            qCWarning(beeCopterlibarchiveLog) << "Failed to finish entry:" << archive_error_string(ext);
            success = false;
            break;
        }
    }
...
// ============================================================================
// beeCopterlibarchive Implementation
// ============================================================================
...
bool ArchiveReader::open(const QString& path, ReaderMode mode)
{
    ...
    if (!_archive) {
        qCWarning(beeCopterlibarchiveLog) << "Failed to create archive reader";
        return false;
    }
...
bool ArchiveReader::open(QIODevice* device, ReaderMode mode)
{
    if (!device || !device->isOpen() || !device->isReadable()) {
        qCWarning(beeCopterlibarchiveLog) << "Device is null, not open, or not readable";
        return false;
    }
    ...
    if (result != ARCHIVE_OK) {
        qCWarning(beeCopterlibarchiveLog) << "Failed to open device:" << archive_error_string(_archive);
        archive_read_free(_archive);
        _archive = nullptr;
        _device = nullptr;
        return false;
...
{
    const bool isResource = beeCopterFileHelper::isQtResource(filePath);

    if (isResource) {
        // Qt resources must be loaded into memory
        QFile inputFile(filePath);
        if (!inputFile.open(QIODevice::ReadOnly)) {
            qCWarning(beeCopterlibarchiveLog) << "Failed to open file:" << filePath << inputFile.errorString();
            return false;
        }
        ...
        if (resourceData.isEmpty()) {
            qCWarning(beeCopterlibarchiveLog) << "File is empty:" << filePath;
            return false;
        }

        if (archive_read_open_memory(a, resourceData.constData(), static_cast<size_t>(resourceData.size())) !=
            ARCHIVE_OK) {
            qCWarning(beeCopterlibarchiveLog) << "Failed to open data:" << archive_error_string(a);
            return false;
        }
    } else {
        // Stream directly from file (memory efficient)
        if (archive_read_open_filename(a, filePath.toLocal8Bit().constData(),
                                       beeCopterFileHelper::optimalBufferSize(filePath)) != ARCHIVE_OK) {
            qCWarning(beeCopterlibarchiveLog) << "Failed to open file:" << filePath << archive_error_string(a);
            return false;
        }
    }
...
bool extractAnyArchive(const QString& archivePath, const QString& outputDirectoryPath, ProgressCallback progress,
                       qint64 maxBytes)
{
    if (!beeCopterFileHelper::ensureDirectoryExists(outputDirectoryPath)) {
        return false;
    }
    ...
    if (stats.totalUncompressedSize > 0) {
        if (!beeCopterFileHelper::hasSufficientDiskSpace(outputDirectoryPath, stats.totalUncompressedSize)) {
            return false;
        }
    }
                          ...
                          qint64 maxBytes)
{
    ...
    if (outputInfo.fileName().isEmpty()) {
        qCWarning(beeCopterlibarchiveLog) << "Invalid output directory path:" << outputDirectoryPath;
        return false;
    }

    const QString parentDirectoryPath = outputInfo.absoluteDir().absolutePath();
    if (!beeCopterFileHelper::ensureDirectoryExists(parentDirectoryPath)) {
        qCWarning(beeCopterlibarchiveLog) << "Failed to create output parent directory:" << parentDirectoryPath;
        return false;
    }
    ...
    if (stats.totalEntries == 0) {
        qCWarning(beeCopterlibarchiveLog) << "Archive is empty or invalid:" << archivePath;
        return false;
    }
    ...
    if (stats.totalUncompressedSize > 0) {
        if (!beeCopterFileHelper::hasSufficientDiskSpace(outputDirectoryPath, stats.totalUncompressedSize * 2)) {
            return false;
        }
    }
    ...
    QTemporaryDir stagingDir(beeCopterFileHelper::joinPath(parentDirectoryPath,
                                                     QStringLiteral("%1.beeCopter_stage_XXXXXX").arg(outputDirectoryName)));
    if (!stagingDir.isValid()) {
        qCWarning(beeCopterlibarchiveLog) << "Failed to create staging directory:" << stagingDir.errorString();
        return false;
    }
    ...
    qCDebug(beeCopterlibarchiveLog) << "Atomic extraction: staging to" << stagingPath;
    ...
    if (!extractArchiveEntries(reader.release(), stagingPath, progress, totalSize, maxBytes)) {
        qCDebug(beeCopterlibarchiveLog) << "Staged extraction failed, cleaning up";
        (void) QDir(stagingPath).removeRecursively();
        return false;
    }
    ...
    if (existingOutputInfo.exists()) {
        if (!existingOutputInfo.isDir()) {
            qCWarning(beeCopterlibarchiveLog) << "Output path exists and is not a directory:" << outputDirectoryPath;
            (void) QDir(stagingPath).removeRecursively();
            return false;
        }

        QTemporaryDir backupDir(beeCopterFileHelper::joinPath(parentDirectoryPath,
                                                        QStringLiteral("%1.beeCopter_backup_XXXXXX").arg(outputDirectoryName)));
        if (!backupDir.isValid()) {
            qCWarning(beeCopterlibarchiveLog) << "Failed to create backup directory placeholder:"
        ...
        if (!parentDir.rename(outputDirectoryName, backupName)) {
            qCWarning(beeCopterlibarchiveLog) << "Failed to move existing output to backup:" << outputDirectoryPath;
            (void) QDir(stagingPath).removeRecursively();
            return false;
        }
    }
    ...
    if (!parentDir.rename(stagingName, outputDirectoryName)) {
        qCWarning(beeCopterlibarchiveLog) << "Failed to commit staged extraction for" << outputDirectoryPath;
        (void) QDir(stagingPath).removeRecursively();

        if (!backupName.isEmpty()) {
            if (!parentDir.rename(backupName, outputDirectoryName)) {
                qCWarning(beeCopterlibarchiveLog) << "Failed to rollback backup for" << outputDirectoryPath;
            }
        }
    ...
    qCDebug(beeCopterlibarchiveLog) << "Atomic extraction committed:" << outputDirectoryPath;
    return true;
}
...
bool extractSingleFile(const QString& archivePath, const QString& fileName, const QString& outputPath)
{
    ...
    while (archive_read_next_header(reader.handle(), &entry) == ARCHIVE_OK) {
        ...
        if (entryName == fileName) {
            beeCopterFileHelper::ensureParentExists(outputPath);
    ...
    qCWarning(beeCopterlibarchiveLog) << "File not found in archive:" << fileName;
    return false;
}
...
{
    if (fileName.isEmpty()) {
        qCWarning(beeCopterlibarchiveLog) << "Empty file name";
        return QByteArray();
    }
    ...
    while (archive_read_next_header(reader.handle(), &entry) == ARCHIVE_OK) {
        ...
        if (entryName == fileName) {
            ...
            if (!result.isEmpty()) {
                qCDebug(beeCopterlibarchiveLog) << "Extracted" << fileName << "to memory:" << result.size() << "bytes";
            }
            return result;
        }
...
bool extractMultipleFiles(const QString& archivePath, const QStringList& fileNames, const QString& outputDirectoryPath)
{
    ...
    while (archive_read_next_header(reader.handle(), &entry) == ARCHIVE_OK) {
        ...
        const QString outputPath = beeCopterFileHelper::joinPath(outputDirectoryPath, entryName);
        beeCopterFileHelper::ensureParentExists(outputPath);
    ...
    if (extractedFiles.size() != targetFiles.size()) {
        for (const QString& name : fileNames) {
            if (!extractedFiles.contains(name)) {
                qCWarning(beeCopterlibarchiveLog) << "File not found in archive:" << name;
            }
        }
        return false;
    }
                      ...
                      QStringList* extractedFiles)
{
    ...
    if (!match) {
        qCWarning(beeCopterlibarchiveLog) << "Failed to create archive_match";
        return false;
    }

    for (const QString& pattern : patterns) {
        if (archive_match_include_pattern(match, pattern.toUtf8().constData()) != ARCHIVE_OK) {
            qCWarning(beeCopterlibarchiveLog) << "Invalid pattern:" << pattern;
    ...
    if (archive_match_path_unmatched_inclusions(match) > 0) {
        ...
        while (archive_match_path_unmatched_inclusions_next(match, &unmatched) == ARCHIVE_OK) {
            qCDebug(beeCopterlibarchiveLog) << "Pattern not matched:" << unmatched;
        }
    }

    archive_match_free(match);

    qCDebug(beeCopterlibarchiveLog) << "Extracted" << matchCount << "files matching patterns from" << archivePath;
    return matchCount > 0;
}
...
bool validateArchive(const QString& archivePath)
{
    ...
    while (archive_read_next_header(reader.handle(), &entry) == ARCHIVE_OK) {
        ...
        if (size < 0) {
            qCWarning(beeCopterlibarchiveLog) << "Validation failed for entry:" << archive_entry_pathname(entry)
...
QStringList listArchiveEntries(const QString& archivePath)
{
    ...
    qCDebug(beeCopterlibarchiveLog) << "Listed" << entries.size() << "entries in" << archivePath;
    return entries;
}
...
QList<ArchiveEntry> listArchiveEntriesDetailed(const QString& archivePath)
{
    ...
    qCDebug(beeCopterlibarchiveLog) << "Listed" << entries.size() << "entries with metadata in" << archivePath;
    return entries;
}
...
ArchiveStats getArchiveStats(const QString& archivePath)
{
    ...
    qCDebug(beeCopterlibarchiveLog) << "Archive stats:" << stats.fileCount << "files," << stats.directoryCount << "dirs,"
...
{
    if (!filter) {
        qCWarning(beeCopterlibarchiveLog) << "No filter provided";
        return false;
    }
    ...
    while (archive_read_next_header(reader.handle(), &entry) == ARCHIVE_OK) {
        ...
        const QString outputPath = beeCopterFileHelper::joinPath(outputDirectoryPath, info.name);
        beeCopterFileHelper::ensureParentExists(outputPath);
    ...
    qCDebug(beeCopterlibarchiveLog) << "Extracted" << extractedCount << "files, skipped" << skippedCount;
    return true;
}
                          ...
                          qint64 maxBytes)
{
    ...
    if (archive_read_next_header(reader.handle(), &entry) != ARCHIVE_OK) {
        qCWarning(beeCopterlibarchiveLog) << "Failed to read header:" << archive_error_string(reader.handle());
        return false;
    }
...
{
    if (data.isEmpty()) {
        qCWarning(beeCopterlibarchiveLog) << "Cannot decompress empty data";
        return {};
    }
    ...
    if (archive_read_open_memory(a, data.constData(), static_cast<size_t>(data.size())) != ARCHIVE_OK) {
        qCWarning(beeCopterlibarchiveLog) << "Failed to open compressed data:" << archive_error_string(a);
        archive_read_free(a);
        return {};
    }
    ...
    if (archive_read_next_header(a, &entry) != ARCHIVE_OK) {
        qCWarning(beeCopterlibarchiveLog) << "Failed to read header:" << archive_error_string(a);
        archive_read_free(a);
        return {};
    }
    ...
    archive_read_free(a);
    qCDebug(beeCopterlibarchiveLog) << "Decompressed" << data.size() << "bytes to" << result.size() << "bytes";
    return result;
}
...
QByteArray extractFileDataFromDevice(QIODevice* device, const QString& fileName)
{
    ...
    while (archive_read_next_header(reader.handle(), &entry) == ARCHIVE_OK) {
        if (QString::fromUtf8(archive_entry_pathname(entry)) == fileName) {
            QByteArray result = readArchiveToMemory(reader.handle(), archive_entry_size(entry));
            if (!result.isEmpty()) {
                qCDebug(beeCopterlibarchiveLog) << "Extracted" << fileName << "from device:" << result.size() << "bytes";
            }
            return result;
        }
        archive_read_data_skip(reader.handle());
...
QByteArray decompressDataFromDevice(QIODevice* device, qint64 maxBytes)
{
    ...
    if (!result.isEmpty()) {
        qCDebug(beeCopterlibarchiveLog) << "Decompressed from device:" << result.size() << "bytes";
    }
    return result;
}
