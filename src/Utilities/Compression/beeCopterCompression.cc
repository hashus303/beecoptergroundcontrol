...
beeCopter_LOGGING_CATEGORY(beeCopterCompressionLog, "Utilities.beeCopterCompression")

namespace beeCopterCompression {
...
static bool validateFileInput(const QString& filePath, Format& format)
{
    ...
    if (!beeCopterFileHelper::exists(filePath)) {
        qCWarning(beeCopterCompressionLog) << "File does not exist:" << filePath;
    ...
    if (format == Format::Auto) {
        ...
        if (format == Format::Auto) {
            qCWarning(beeCopterCompressionLog) << "Could not detect format for:" << filePath;
...
static bool validateArchiveInput(const QString& archivePath, Format& format)
{
    ...
    if (!isArchiveFormat(format)) {
        qCWarning(beeCopterCompressionLog) << "Not an archive format:" << formatName(format);
...
static bool validateDeviceInput(QIODevice* device)
{
    ...
    if (!device || !device->isOpen() || !device->isReadable()) {
        qCWarning(beeCopterCompressionLog) << "Device is null, not open, or not readable";
...
/// Capture format detection info from beeCopterlibarchive after an operation
static void captureFormatInfo()
{
    setFormatInfo(beeCopterlibarchive::lastDetectedFormatName(), beeCopterlibarchive::lastDetectedFilterName());
}
...
Format detectFormat(const QString& filePath, bool useContentFallback)
{
    ...
    if (useContentFallback && beeCopterFileHelper::exists(filePath)) {
        format = detectFormatFromFile(filePath);
        if (format != Format::Auto) {
            qCDebug(beeCopterCompressionLog) << "Format detected from content:" << formatName(format) << "for" << filePath;
        }
    }
...
{
    // Handle Qt resources
    if (beeCopterFileHelper::isQtResource(filePath)) {
        QFile file(filePath);
    ...
    if (mimeType.isValid() && mimeType.name() != QLatin1String("application/octet-stream")) {
        ...
        if (format != Format::Auto) {
            qCDebug(beeCopterCompressionLog) << "MIME detection:" << mimeType.name() << "->" << formatName(format);
            return format;
        }
    }
...
Format detectFormatFromData(const QByteArray& data)
{
    ...
    if (mimeType.isValid() && mimeType.name() != QLatin1String("application/octet-stream")) {
        Format format = formatFromMimeType(mimeType.name());
        if (format != Format::Auto) {
            qCDebug(beeCopterCompressionLog) << "MIME fallback detection:" << mimeType.name() << "->" << formatName(format);
            return format;
        }
    }
                    ...
                    qint64 maxDecompressedBytes)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Decompressing" << inputPath << "to" << actualOutput << "using" << formatName(format);

    // Single-file compression formats
    if (isCompressionFormat(format)) {
        const bool success =
            beeCopterlibarchive::decompressSingleFile(inputPath, actualOutput, progress, maxDecompressedBytes);
        captureFormatInfo();
        if (!success) {
    ...
    if (isArchiveFormat(format)) {
        qCWarning(beeCopterCompressionLog) << formatName(format) << "is an archive format; use extractArchive() instead";
        return extractArchive(inputPath, actualOutput, format, progress, maxDecompressedBytes);
    }

    qCWarning(beeCopterCompressionLog) << "Unsupported decompression format:" << formatName(format);
...
QString decompressIfNeeded(const QString& filePath, const QString& outputPath, bool removeOriginal)
{
    ...
    if (!decompressFile(filePath, actualOutput)) {
        qCWarning(beeCopterCompressionLog) << "Decompression failed:" << filePath;
        return QString();
    }

    if (removeOriginal && !QFile::remove(filePath)) {
        qCWarning(beeCopterCompressionLog) << "Failed to remove original file:" << filePath;
    }
...
{
    if (data.isEmpty()) {
        qCWarning(beeCopterCompressionLog) << "Cannot decompress empty data";
        return {};
    }
    ...
    if (format == Format::Auto) {
        ...
        if (format == Format::Auto) {
            qCWarning(beeCopterCompressionLog) << "Could not detect format from data";
            return {};
        }
    }

    if (!isCompressionFormat(format)) {
        qCWarning(beeCopterCompressionLog) << "Invalid decompression format:" << formatName(format);
        setError(Error::UnsupportedFormat, formatName(format) + QStringLiteral(" is not a compression format"));
        return {};
    }

    QByteArray result = beeCopterlibarchive::decompressDataFromMemory(data, maxDecompressedBytes);
    captureFormatInfo();
    if (result.isEmpty()) {
                    ...
                    ProgressCallback progress, qint64 maxDecompressedBytes)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Extracting" << formatName(format) << "archive" << archivePath << "to"
                               << outputDirectoryPath;

    const bool success =
        beeCopterlibarchive::extractAnyArchive(archivePath, outputDirectoryPath, progress, maxDecompressedBytes);
    captureFormatInfo();
    if (!success) {
                          ...
                          ProgressCallback progress, qint64 maxDecompressedBytes)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Atomically extracting" << formatName(format) << "archive" << archivePath
                               << "to" << outputDirectoryPath;

    const bool success =
        beeCopterlibarchive::extractArchiveAtomic(archivePath, outputDirectoryPath, progress, maxDecompressedBytes);
    captureFormatInfo();
    if (!success) {
                            ...
                            ProgressCallback progress, qint64 maxDecompressedBytes)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Extracting archive with filter:" << archivePath;

    const bool success =
        beeCopterlibarchive::extractWithFilter(archivePath, outputDirectoryPath, filter, progress, maxDecompressedBytes);
    captureFormatInfo();
    if (!success) {
        setError(Error::IoError, QStringLiteral("Failed to extract archive: ") + archivePath);
    }
...
QStringList listArchive(const QString& archivePath, Format format)
{
    ...
    QStringList entries = beeCopterlibarchive::listArchiveEntries(archivePath);
...
QList<ArchiveEntry> listArchiveDetailed(const QString& archivePath, Format format)
{
    ...
    QList<ArchiveEntry> entries = beeCopterlibarchive::listArchiveEntriesDetailed(archivePath);
...
ArchiveStats getArchiveStats(const QString& archivePath, Format format)
{
    ...
    return beeCopterlibarchive::getArchiveStats(archivePath);
}
...
bool validateArchive(const QString& archivePath, Format format)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Validating archive:" << archivePath;
    return beeCopterlibarchive::validateArchive(archivePath);
}

bool fileExists(const QString& archivePath, const QString& fileName, Format format)
{
    if (fileName.isEmpty()) {
        qCWarning(beeCopterCompressionLog) << "File name cannot be empty";
        return false;
    }
    if (!validateArchiveInput(archivePath, format)) {
        return false;
    }
    return beeCopterlibarchive::fileExistsInArchive(archivePath, fileName);
}
...
bool extractFile(const QString& archivePath, const QString& fileName, const QString& outputPath, Format format)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Extracting" << fileName << "from" << archivePath << "to" << actualOutput;
    return beeCopterlibarchive::extractSingleFile(archivePath, fileName, actualOutput);
}
...
QByteArray extractFileData(const QString& archivePath, const QString& fileName, Format format)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Extracting" << fileName << "from" << archivePath << "to memory";
    return beeCopterlibarchive::extractFileToMemory(archivePath, fileName);
}
                  ...
                  Format format)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Extracting" << fileNames.size() << "files from" << archivePath;
    return beeCopterlibarchive::extractMultipleFiles(archivePath, fileNames, outputDirectoryPath);
}
                      ...
                      QStringList* extractedFiles, Format format)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Extracting files matching patterns" << patterns << "from" << archivePath;
    const bool success = beeCopterlibarchive::extractByPattern(archivePath, patterns, outputDirectoryPath, extractedFiles);
    if (!success) {
                          ...
                          qint64 maxDecompressedBytes)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Decompressing from device to" << outputPath;
    const bool success = beeCopterlibarchive::decompressFromDevice(device, outputPath, progress, maxDecompressedBytes);
    captureFormatInfo();
    if (!success) {
...
QByteArray decompressFromDevice(QIODevice* device, qint64 maxDecompressedBytes)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Decompressing from device to memory";
    QByteArray result = beeCopterlibarchive::decompressDataFromDevice(device, maxDecompressedBytes);
    captureFormatInfo();
    if (result.isEmpty()) {
        setError(Error::IoError, QStringLiteral("Failed to decompress from device"));
    }
                       ...
                       qint64 maxDecompressedBytes)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Extracting archive from device to" << outputDirectoryPath;
    const bool success = beeCopterlibarchive::extractFromDevice(device, outputDirectoryPath, progress, maxDecompressedBytes);
    captureFormatInfo();
    if (!success) {
...
QByteArray extractFileDataFromDevice(QIODevice* device, const QString& fileName)
{
    ...
    qCDebug(beeCopterCompressionLog) << "Extracting" << fileName << "from device to memory";
    QByteArray result = beeCopterlibarchive::extractFileDataFromDevice(device, fileName);
    captureFormatInfo();
    if (result.isEmpty()) {
