...
/// @file beeCopterDecompressDevice.h
...
Q_DECLARE_LOGGING_CATEGORY(beeCopterDecompressDeviceLog)
...
/// Example usage:
/// @code
/// beeCopterDecompressDevice device("data.json.gz");
/// device.open(QIODevice::ReadOnly);
/// QTextStream stream(&device);
/// QString content = stream.readAll();
/// @endcode
class beeCopterDecompressDevice : public beeCopterArchiveDeviceBase
{
    Q_OBJECT
...
public:
    ...
    explicit beeCopterDecompressDevice(QIODevice *source, QObject *parent = nullptr);

    /// Construct from file path
    /// @param filePath Path to compressed file (or Qt resource path :/)
    /// @param parent QObject parent
    explicit beeCopterDecompressDevice(const QString &filePath, QObject *parent = nullptr);

    ~beeCopterDecompressDevice() override = default;
