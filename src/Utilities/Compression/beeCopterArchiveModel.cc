...
beeCopter_LOGGING_CATEGORY(beeCopterArchiveModelLog, "Utilities.beeCopterArchiveModel")

beeCopterArchiveModel::beeCopterArchiveModel(QObject *parent)
    : QAbstractListModel(parent)
{
}

beeCopterArchiveModel::~beeCopterArchiveModel() = default;

void beeCopterArchiveModel::setArchivePath(const QString &path)
{
...
void beeCopterArchiveModel::setArchiveUrl(const QUrl &url)
{
    const QString path = beeCopterFileHelper::toLocalPath(url);
    if (!path.isEmpty() && !beeCopterFileHelper::isLocalPath(path)) {
        qCWarning(beeCopterArchiveModelLog) << "Unsupported archive URL:" << url;
        setArchivePath({});
        return;
    }
...
void beeCopterArchiveModel::setFilterMode(FilterMode mode)
{
    if (_filterMode == mode) {
        return;
    }
...
int beeCopterArchiveModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid()) {
        return 0;
    }
...
QVariant beeCopterArchiveModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid() || index.row() < 0 || index.row() >= _filteredEntries.size()) {
        return {};
    }

    const beeCopterCompression::ArchiveEntry &entry = _filteredEntries.at(index.row());
...
QHash<int, QByteArray> beeCopterArchiveModel::roleNames() const
{
...
void beeCopterArchiveModel::refresh()
{
    _loadArchive();
}

void beeCopterArchiveModel::clear()
{
...
QVariantMap beeCopterArchiveModel::get(int index) const
{
    QVariantMap result;
    ...
    const beeCopterCompression::ArchiveEntry &entry = _filteredEntries.at(index);
...
bool beeCopterArchiveModel::contains(const QString &fileName) const
{
    if (fileName.isEmpty()) {
        return false;
    }
...
QString beeCopterArchiveModel::formatSize(qint64 bytes)
{
    if (bytes < 0) {
...
void beeCopterArchiveModel::_loadArchive()
{
    clear();
    ...
    qCDebug(beeCopterArchiveModelLog) << "Loading archive:" << _archivePath;

    // Load entries using beeCopterCompression (already sorted with QCollator)
    QList<beeCopterCompression::ArchiveEntry> entries = beeCopterCompression::listArchiveDetailed(_archivePath);

    if (entries.isEmpty() && !_archivePath.isEmpty()) {
        // Check if it was an error or just empty archive
        if (!beeCopterCompression::validateArchive(_archivePath)) {
            _setErrorString(beeCopterCompression::lastErrorString());
            _setLoading(false);
    ...
    qCDebug(beeCopterArchiveModelLog) << "Loaded" << _allEntries.size() << "entries:"
...
void beeCopterArchiveModel::_applyFilter()
{
    beginResetModel();
...
void beeCopterArchiveModel::_setLoading(bool loading)
{
    if (_loading == loading) {
        return;
    }
...
void beeCopterArchiveModel::_setErrorString(const QString &error)
{
