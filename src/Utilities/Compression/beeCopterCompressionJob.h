...
/// @file beeCopterCompressionJob.h
...
Q_DECLARE_LOGGING_CATEGORY(beeCopterCompressionJobLog)
...
/// Example C++ usage:
/// @code
/// auto *job = new beeCopterCompressionJob(this);
/// connect(job, &beeCopterCompressionJob::finished, this, [](bool success) {
///     qDebug() << "Extraction" << (success ? "succeeded" : "failed");
/// });
/// connect(job, &beeCopterCompressionJob::progressChanged, this, [](qreal p) {
...
/// @code
/// QFuture<bool> future = beeCopterCompressionJob::extractArchiveAsync("archive.zip", "/output");
...
/// @endcode
class beeCopterCompressionJob : public QObject
{
    Q_OBJECT
    Q_DISABLE_COPY_MOVE(beeCopterCompressionJob)
...
public:
    ...
    explicit beeCopterCompressionJob(QObject *parent = nullptr);
    ~beeCopterCompressionJob() override;
...
private:
    using WorkFunction = std::function<bool(beeCopterCompression::ProgressCallback)>;
