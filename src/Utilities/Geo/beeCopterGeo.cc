...
beeCopter_LOGGING_CATEGORY(beeCopterGeoLog, "Utilities.beeCopterGeo")

namespace beeCopterGeo
{
...
int convertGeoToUTM(const QGeoCoordinate &coord, double &easting, double &northing)
{
    try {
        ...
        qCDebug(beeCopterGeoLog) << e.what();
        return 0;
    }
}
...
QList<QGeoCoordinate> interpolatePath(const QGeoCoordinate &from, const QGeoCoordinate &to, int numPoints)
{
    ...
    if (numPoints < 2) {
        ...
        qCWarning(beeCopterGeoLog) << "interpolatePath: numPoints" << numPoints << "exceeds maximum, clamping to" << kMaxPoints;
