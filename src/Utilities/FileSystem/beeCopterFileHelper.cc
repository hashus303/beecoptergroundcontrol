...
Q_DECLARE_LOGGING_CATEGORY(beeCopterFileHelperLog)
Q_LOGGING_CATEGORY(beeCopterFileHelperLog, "Utilities.beeCopterFileHelper")

namespace beeCopterFileHelper {
...
QByteArray readFile(const QString &filePath, QString *errorString, qint64 maxBytes)
{
    ...
    if (beeCopterCompression::isCompressedFile(filePath)) {
        beeCopterDecompressDevice decompressor(filePath);
...
bool copyDirectoryRecursively(const QString &sourcePath, const QString &destPath)
{
    ...
    if (!sourceDir.exists()) {
        qCWarning(beeCopterFileHelperLog) << "Source directory doesn't exist:" << sourcePath;
        return false;
    }

    if (!ensureDirectoryExists(destPath)) {
        qCWarning(beeCopterFileHelperLog) << "Failed to create destination directory:" << destPath;
        return false;
    }
    ...
    for (const QString &fileName : files) {
        ...
        if (!QFile::copy(srcFilePath, dstFilePath)) {
            qCWarning(beeCopterFileHelperLog) << "Failed to copy file:" << srcFilePath << "to" << dstFilePath;
            return false;
        }
    }
...
bool moveFileOrCopy(const QString &sourcePath, const QString &destPath)
{
    if (sourcePath.isEmpty() || destPath.isEmpty()) {
        qCWarning(beeCopterFileHelperLog) << "moveFileOrCopy: empty path";
        return false;
    }
    ...
    qCDebug(beeCopterFileHelperLog) << "moveFileOrCopy: rename failed, using copy for:" << sourcePath;
    ...
    if (!copySuccess) {
        qCWarning(beeCopterFileHelperLog) << "moveFileOrCopy: failed to move:" << sourcePath << "to" << destPath;
    }
...
{
    if (filePath.isEmpty()) {
        qCWarning(beeCopterFileHelperLog) << "atomicWrite: file path is empty";
        return false;
    }

    // Ensure parent directory exists
    if (!ensureParentExists(filePath)) {
        qCWarning(beeCopterFileHelperLog) << "atomicWrite: failed to create parent directory for:" << filePath;
        return false;
    }
    ...
    if (!file.open(QIODevice::WriteOnly)) {
        qCWarning(beeCopterFileHelperLog) << "atomicWrite: failed to open:" << filePath
                                    << "-" << file.errorString();
        return false;
    }

    if (file.write(data) != data.size()) {
        qCWarning(beeCopterFileHelperLog) << "atomicWrite: write failed:" << file.errorString();
        file.cancelWriting();
        return false;
    }

    if (!file.commit()) {
        qCWarning(beeCopterFileHelperLog) << "atomicWrite: commit failed:" << file.errorString();
        return false;
    }
...
qint64 availableDiskSpace(const QString &path)
{
    ...
    if (!storage.isValid()) {
        qCDebug(beeCopterFileHelperLog) << "availableDiskSpace: cannot determine storage for:" << path;
        return -1;
    }
...
bool hasSufficientDiskSpace(const QString &path, qint64 requiredBytes, double margin)
{
    ...
    if (bytesAvailable < 0) {
        qCDebug(beeCopterFileHelperLog) << "hasSufficientDiskSpace: cannot determine disk space, proceeding anyway";
    ...
    if (bytesAvailable < bytesRequired) {
        ...
        qCWarning(beeCopterFileHelperLog) << "Insufficient disk space:"
...
QString toLocalPath(const QUrl &url)
{
    ...
    qCDebug(beeCopterFileHelperLog) << "toLocalPath: URL scheme not supported for local access:" << scheme;
    return url.toString();
}
...
{
    if (filePath.isEmpty()) {
        qCWarning(beeCopterFileHelperLog) << "computeFileHash: empty file path";
        return QString();
    }

    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly)) {
        qCWarning(beeCopterFileHelperLog) << "computeFileHash: failed to open file:" << filePath
                                    << "-" << file.errorString();
        return QString();
    }
    ...
    if (!hash.addData(&file)) {
        qCWarning(beeCopterFileHelperLog) << "computeFileHash: failed to read file:" << filePath;
        return QString();
    }
...
{
    if (filePath.isEmpty()) {
        qCWarning(beeCopterFileHelperLog) << "computeDecompressedFileHash: empty file path";
        return {};
    }
    ...
    beeCopterDecompressDevice decompressor(filePath);
    if (!decompressor.open(QIODevice::ReadOnly)) {
        qCWarning(beeCopterFileHelperLog) << "computeDecompressedFileHash: failed to open:" << filePath;
        return {};
    }
    ...
    while (true) {
        ...
        if (buffer.isEmpty()) {
            ...
            qCWarning(beeCopterFileHelperLog) << "computeDecompressedFileHash: read error";
            decompressor.close();
            return {};
        }
                    ...
                    QCryptographicHash::Algorithm algorithm)
{
    if (expectedHash.isEmpty()) {
        qCWarning(beeCopterFileHelperLog) << "verifyFileHash: empty expected hash";
        return false;
    }
    ...
    if (!match) {
        qCWarning(beeCopterFileHelperLog) << "verifyFileHash: hash mismatch for" << filePath
...
{
    QString name = templateName.isEmpty() ? QStringLiteral("beeCopter_XXXXXX") : templateName;
...
QString uniqueTempPath(const QString &templateName)
{
    ...
    if (!temp.open()) {
        qCWarning(beeCopterFileHelperLog) << "uniqueTempPath: failed to create temp file";
        return {};
    }
...
std::unique_ptr<QTemporaryFile> createTempFile(const QByteArray &data, const QString &templateName)
{
    ...
    if (!temp->open()) {
        qCWarning(beeCopterFileHelperLog) << "createTempFile: failed to create temp file";
        return nullptr;
    }

    if (temp->write(data) != data.size()) {
        qCWarning(beeCopterFileHelperLog) << "createTempFile: failed to write data";
        return nullptr;
    }
    ...
    qCDebug(beeCopterFileHelperLog) << "createTempFile: created" << temp->fileName()
...
std::unique_ptr<QTemporaryFile> createTempCopy(const QString &sourcePath, const QString &templateName)
{
    if (sourcePath.isEmpty()) {
        qCWarning(beeCopterFileHelperLog) << "createTempCopy: source path is empty";
        return nullptr;
    }

    QFile source(sourcePath);
    if (!source.open(QIODevice::ReadOnly)) {
        qCWarning(beeCopterFileHelperLog) << "createTempCopy: failed to open source:" << sourcePath
                                    << "-" << source.errorString();
        return nullptr;
    }
    ...
    if (temp) {
        qCDebug(beeCopterFileHelperLog) << "createTempCopy: copied" << sourcePath << "to" << temp->fileName();
    }
    return temp;
}
...
bool replaceFileFromTemp(QTemporaryFile *tempFile, const QString &targetPath, const QString &backupPath)
{
    if (!tempFile || !tempFile->isOpen()) {
        qCWarning(beeCopterFileHelperLog) << "replaceFileFromTemp: temp file is null or not open";
        return false;
    }

    if (targetPath.isEmpty()) {
        qCWarning(beeCopterFileHelperLog) << "replaceFileFromTemp: target path is empty";
        return false;
    }

    // Ensure parent directory exists
    if (!ensureParentExists(targetPath)) {
        qCWarning(beeCopterFileHelperLog) << "replaceFileFromTemp: failed to create parent directory";
        return false;
    }
    ...
    if (!backupPath.isEmpty() && QFile::exists(targetPath)) {
        ...
        if (!QFile::copy(targetPath, backupPath)) {
            qCWarning(beeCopterFileHelperLog) << "replaceFileFromTemp: failed to create backup";
            return false;
        }
        qCDebug(beeCopterFileHelperLog) << "replaceFileFromTemp: backed up to" << backupPath;
    }
    ...
    if (!moveFileOrCopy(tempPath, targetPath)) {
        qCWarning(beeCopterFileHelperLog) << "replaceFileFromTemp: failed to move temp file to target";
        QFile::remove(tempPath);  // Clean up on failure
        return false;
    }

    qCDebug(beeCopterFileHelperLog) << "replaceFileFromTemp: replaced" << targetPath;
    return true;
}
