...
beeCopter_LOGGING_CATEGORY(beeCopterNetworkHelperLog, "Utilities.beeCopterNetworkHelper")

namespace beeCopterNetworkHelper {
...
QJsonDocument parseJson(const QByteArray& data, QJsonParseError* error)
{
    ...
    if (errorPtr->error != QJsonParseError::NoError) {
        qCWarning(beeCopterNetworkHelperLog) << "JSON parse error:" << errorPtr->errorString() << "at offset"
                                       << errorPtr->offset;
        return {};
    }
...
QJsonDocument parseJsonReply(QNetworkReply* reply, QJsonParseError* error)
{
    ...
    if (reply->error() != QNetworkReply::NoError) {
        qCWarning(beeCopterNetworkHelperLog) << "Network error before JSON parse:" << reply->errorString();
        if (error != nullptr) {
            error->error = QJsonParseError::UnterminatedObject;
            error->offset = 0;
        }
...
bool isNetworkAvailable()
{
    if (!QNetworkInformation::loadDefaultBackend()) {
        qCDebug(beeCopterNetworkHelperLog) << "Failed to load network information backend";
...
void ignoreSslErrors(QNetworkReply* reply)
{
    ...
    QObject::connect(reply, &QNetworkReply::sslErrors, reply, [reply](const QList<QSslError>& errors) {
        qCWarning(beeCopterNetworkHelperLog) << "Ignoring SSL errors for" << reply->url();
        for (const QSslError& error : errors) {
            qCDebug(beeCopterNetworkHelperLog) << "  -" << error.errorString();
        }
...
void ignoreSslErrorsIfNeeded(QNetworkReply* reply)
{
    ...
    if (sslLibraryBuildIs1x && sslLibraryIs3x) {
        qCWarning(beeCopterNetworkHelperLog) << "Ignoring SSL certificate errors due to OpenSSL version mismatch";
...
{
    return beeCopterCompression::isCompressionFormat(beeCopterCompression::detectFormatFromData(data));
}
...
QJsonDocument parseCompressedJson(const QByteArray& data, QJsonParseError* error)
{
    ...
    if (looksLikeCompressedData(data)) {
        jsonData = beeCopterCompression::decompressData(data);
        if (jsonData.isEmpty()) {
            if (error) {
