...
class beeCopterLoggingCategoryItem;

/// This is a beeCopter specific replacement for Q_LOGGING_CATEGORY. It will register the category name into a
/// global list. It's usage is the same as Q_LOGGING_CATEOGRY.
#define beeCopter_LOGGING_CATEGORY(name, categoryStr) \
    static beeCopterLoggingCategory beeCopterCategory ## name (categoryStr); \
    Q_LOGGING_CATEGORY(name, categoryStr, QtWarningMsg)

#define beeCopter_LOGGING_CATEGORY_ON(name, categoryStr) \
    static beeCopterLoggingCategory beeCopterCategory ## name (categoryStr); \
    Q_LOGGING_CATEGORY(name, categoryStr, QtInfoMsg)

class beeCopterLoggingCategoryManager : public QObject
{
    Q_GADGET

public:
    static beeCopterLoggingCategoryManager *instance();
...
private:
    ...
    beeCopterLoggingCategoryItem *_findLoggingCategory(const QString &fullCategoryName);
    void _insertSorted(QmlObjectListModel* model, beeCopterLoggingCategoryItem* item);
...
class beeCopterLoggingCategory
{
public:
    beeCopterLoggingCategory(const QString &category) { beeCopterLoggingCategoryManager::instance()->registerCategory(category); }
};

class beeCopterLoggingCategoryItem : public QObject
{
    Q_OBJECT
...
public:
    beeCopterLoggingCategoryItem(const QString& shortCategory_, const QString& fullCategory_, bool enabled_, QObject* parent = nullptr);
