...
beeCopter_LOGGING_CATEGORY(beeCopterSerialPortInfoLog, "Comms.beeCopterSerialPortInfo")

bool beeCopterSerialPortInfo::_jsonLoaded = false;
bool beeCopterSerialPortInfo::_jsonDataValid = false;
QList<beeCopterSerialPortInfo::BoardInfo_t> beeCopterSerialPortInfo::_boardInfoList;
QList<beeCopterSerialPortInfo::BoardRegExpFallback_t> beeCopterSerialPortInfo::_boardDescriptionFallbackList;
QList<beeCopterSerialPortInfo::BoardRegExpFallback_t> beeCopterSerialPortInfo::_boardManufacturerFallbackList;

beeCopterSerialPortInfo::beeCopterSerialPortInfo()
    : QSerialPortInfo()
{
    qCDebug(beeCopterSerialPortInfoLog) << this;
}

beeCopterSerialPortInfo::beeCopterSerialPortInfo(const QSerialPort &port)
...
bool beeCopterSerialPortInfo::_loadJsonData()
{
    if (_jsonLoaded) {
    ...
    QString errorString;
    int version;
    const QJsonObject json = JsonHelper::openInternalbeeCopterJsonFile(QStringLiteral(":/json/USBBoardInfo.json"), QString(_jsonFileTypeValue), 1, 1, version, errorString);
    if (!errorString.isEmpty()) {
        qCWarning(beeCopterSerialPortInfoLog) << "Internal Error:" << errorString;
        return false;
    }
    ...
    if (!JsonHelper::validateKeys(json, rootKeyInfoList, errorString)) {
        qCWarning(beeCopterSerialPortInfoLog) << errorString;
        return false;
    }
    ...
    for (const QJsonValue &jsonValue : rgBoardInfo) {
        if (!jsonValue.isObject()) {
            qCWarning(beeCopterSerialPortInfoLog) << "Entry in boardInfo array is not object";
            return false;
        }
        ...
        if (boardInfo.boardType == BoardTypeUnknown) {
            qCWarning(beeCopterSerialPortInfoLog) << "Bad board class" << boardObject[_jsonBoardClassKey].toString();
            return false;
        }
    ...
    for (const QJsonValue &jsonValue : rgBoardDescriptionFallback) {
        if (!jsonValue.isObject()) {
            qCWarning(beeCopterSerialPortInfoLog) << "Entry in boardFallback array is not object";
            return false;
        }
        ...
        if (boardFallback.boardType == BoardTypeUnknown) {
            qCWarning(beeCopterSerialPortInfoLog) << "Bad board class" << fallbackObject[_jsonBoardClassKey].toString();
            return false;
        }
...
beeCopterSerialPortInfo::BoardType_t beeCopterSerialPortInfo::_boardClassStringToType(const QString &boardClass)
{
...
bool beeCopterSerialPortInfo::getBoardInfo(beeCopterSerialPortInfo::BoardType_t &boardType, QString &name) const
{
...
QString beeCopterSerialPortInfo::_boardTypeToString(BoardType_t boardType)
{
    switch (boardType) {
...
QList<beeCopterSerialPortInfo> beeCopterSerialPortInfo::availablePorts()
{
    QList<beeCopterSerialPortInfo> list;
    ...
    for (const QSerialPortInfo &portInfo : availablePorts) {
        ...
        const beeCopterSerialPortInfo *const beeCopterPortInfo = reinterpret_cast<const beeCopterSerialPortInfo*>(&portInfo);
        list << *beeCopterPortInfo;
    }

    return list;
}

bool beeCopterSerialPortInfo::isBootloader() const
{
...
bool beeCopterSerialPortInfo::isBlackCube() const
{
    return description().contains(QStringLiteral("CubeBlack"));
}

bool beeCopterSerialPortInfo::isSystemPort(const QSerialPortInfo &port)
{
#ifdef Q_OS_MACOS
...
bool beeCopterSerialPortInfo::canFlash() const
{
    BoardType_t boardType;
    QString name;
    if (!getBoardInfo(boardType, name)) {
