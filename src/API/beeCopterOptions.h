...
Q_DECLARE_LOGGING_CATEGORY(beeCopterFlyViewOptionsLog)

class beeCopterFlyViewOptions : public QObject
{
    Q_OBJECT
    QML_ELEMENT
    QML_UNCREATABLE("")
...
public:
    explicit beeCopterFlyViewOptions(beeCopterOptions *options, QObject *parent = nullptr);
    ~beeCopterFlyViewOptions();
...
protected:
    ...
    const beeCopterOptions *_options = nullptr;
};

/*===========================================================================*/

Q_DECLARE_LOGGING_CATEGORY(beeCopterOptionsLog)

class beeCopterOptions : public QObject
{
    Q_OBJECT
    QML_ELEMENT
    QML_UNCREATABLE("")
    ...
    Q_PROPERTY(const beeCopterFlyViewOptions *flyView     READ flyViewOptions                 CONSTANT)
...
public:
    explicit beeCopterOptions(QObject *parent = nullptr);
    ~beeCopterOptions();

    /// Should beeCopter hide its settings menu and colapse it into one single menu (Settings and Vehicle Setup)?
    /// @return true if beeCopter should consolidate both menus into one.
    ...
    virtual QUrl preFlightChecklistUrl() const { return QUrl::fromUserInput(QStringLiteral("qrc:/qml/beeCopter/FlyView/PreFlightCheckList.qml")); }
    ...
    virtual const beeCopterFlyViewOptions *flyViewOptions() const { return _defaultFlyViewOptions; }
...
protected:
    const beeCopterFlyViewOptions *_defaultFlyViewOptions = nullptr;
};
