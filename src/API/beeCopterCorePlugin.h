...
Q_DECLARE_LOGGING_CATEGORY(beeCopterCorePluginLog)

class beeCopterCorePlugin : public QObject
{
    Q_OBJECT
    QML_UNCREATABLE("")
    Q_MOC_INCLUDE("beeCopterOptions.h")
    ...
    Q_PROPERTY(const beeCopterOptions *options                READ options                                                        CONSTANT)
...
public:
    explicit beeCopterCorePlugin(QObject *parent = nullptr);
    virtual ~beeCopterCorePlugin();

    static beeCopterCorePlugin *instance();
    ...
    /// Global options
    /// @return An instance of beeCopterOptions
    virtual beeCopterOptions *options();
    ...
    virtual void paletteOverride(const QString &colorName, beeCopterPalette::PaletteColorInfo_t &colorInfo) { Q_UNUSED(colorName); Q_UNUSED(colorInfo); };
    ...
    /// Default beeCopter mainline implemenentation returns beeCopter Stable file location. Default beeCopter custom build code returns QString().
...
#else
    virtual QString stableVersionCheckFileUrl() const { return QStringLiteral("https://s3-us-west-2.amazonaws.com/beeCopter/latest/beeCopter.version.txt"); }
#endif
    ...
    virtual QString stableDownloadLocation() const { return QStringLiteral("beeCopter.com"); }
...
private:
    ...
    beeCopterOptions *_defaultOptions = nullptr;
