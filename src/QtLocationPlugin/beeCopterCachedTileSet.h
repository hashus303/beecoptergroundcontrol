...
Q_DECLARE_LOGGING_CATEGORY(beeCopterCachedTileSetLog)

struct beeCopterTile;
class beeCopterMapEngineManager;
class QNetworkAccessManager;

class beeCopterCachedTileSet : public QObject
{
    Q_OBJECT
    Q_MOC_INCLUDE("beeCopterTile.h")
...
public:
    explicit beeCopterCachedTileSet(const QString &name, QObject *parent = nullptr);
    ~beeCopterCachedTileSet();
    ...
    void setManager(beeCopterMapEngineManager *mgr) { _manager = mgr; }
...
private slots:
    void _tileListFetched(const QQueue<beeCopterTile*> &tiles);
...
private:
    ...
    QQueue<beeCopterTile*> _tilesToDownload;
    beeCopterMapEngineManager *_manager = nullptr;
