...
Q_DECLARE_LOGGING_CATEGORY(QGeoFileTileCachebeeCopterLog)

class beeCopterFetchTileTask;

class QGeoFileTileCachebeeCopter : public QGeoFileTileCache
{
    Q_OBJECT

public:
    explicit QGeoFileTileCachebeeCopter(const QVariantMap &parameters, QObject *parent = nullptr);
    ~QGeoFileTileCachebeeCopter();
    ...
    static beeCopterFetchTileTask *createFetchTileTask(const QString &type, int x, int y, int z);
