...
class QQuickWindow;
class beeCopterImageProvider;
class beeCopterApplication;
class QEvent;
...
#undef qApp
#endif
#define qApp (static_cast<beeCopterApplication*>(QApplication::instance()))

#if defined(qGuiApp)
#undef qGuiApp
#endif
#define qGuiApp (static_cast<beeCopterApplication*>(QGuiApplication::instance()))

#define beeCopterApp() qApp

Q_DECLARE_LOGGING_CATEGORY(beeCopterApplicationLog)
...
class beeCopterApplication : public QApplication
{
    Q_OBJECT
...
public:
    beeCopterApplication(int &argc, char *argv[], const beeCopterCommandLineParser::CommandLineParseResult &args);
    ~beeCopterApplication();

    /// Sets the persistent flag to delete all settings the next time beeCopter is started.
    static void deleteAllSettingsNextBoot();

    /// Clears the persistent flag to delete all settings the next time beeCopter is started.
...
private slots:
    ...
    void _beeCopterCurrentStableVersionDownloadComplete(bool success, const QString &localFile, const QString &errorMsg);
...
private:
    ...
    QTranslator _beeCopterTranslatorSourceCode;           ///< translations for source code C++/Qml
    QTranslator _beeCopterTranslatorQtLibs;               ///< tranlsations for Qt libraries
    QLocale _locale;
    bool _error = false;
    ...
    const QString _beeCopterImageProviderId = QStringLiteral("beeCopterImages");
};
